...
class OpinionSpreadModel(Model):
    ...
    def __init__(
        ...
        # 随机设置初始感染源 - Mesa 2.x agents是列表，按unique_id查找
        ...
        self.datacollector = DataCollector(
            model_reporters={
                ...
                "Intervention": lambda m: m.intervention.get_current_effect(m.current_step),
            }
        )
    ...
    def _count(self, state):
        """统计特定状态的agent数量 - Mesa 2.x agents是列表"""
        return sum(1 for a in self.schedule.agents if a.state == state)

    def get_agent_states(self):
        """返回 {node_id: state} 字典，供网络快照图使用 - Mesa 2.x agents是列表"""
